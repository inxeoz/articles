...
def _verify_otp_get_page_access():
        """
        Parse `page_access` from request (frappe.form_dict). Returns:
        - None   -> no filter present
        - set(...) -> set of page identifiers (strings)
        ...
        wiki_page_list = frappe.form_dict.get("wiki_page_list")
        if not wiki_page_list:
            return []
        try:
            doc = frappe.get_doc("wiki_page_list", wiki_page_list).as_dict()
            #print(f"doc --> {doc}") 
            
            page_list = doc["page_list"]
            page_list_json = json.loads(page_list)
            #print(type(page_list_json))
...
class WikiPage(WebsiteGenerator):
    ...
    def is_page_accessiable(self, page_name=None):
        
        if page_name is None:
            page_name = self.name     
            
            print(f"---<<<<>>>>{page_name} self.name") 
        
        page_list = _verify_otp_get_page_access()
    ...
    def get_context(self, context):
        ...
        if frappe.session.user != "Guest":
            ...
            if not self.is_page_accessiable():
                print(f"this page is not accessiable ")
                frappe.throw("this page is not accessiable ")
    ...
    def get_sidebar_items(self):
        ...
        for sidebar_item in wiki_sidebar:
            ...
            if frappe.session.user != "Guest":
                ## make sure the acccess happen using param in url
                if not self.is_page_accessiable(wiki_page.name):
                    continue
                
                
            
            # if permitted & not self.is_page_accessiable(wiki_page.name):
            #     # continue
            #     print()
